'use client'

import { useEffect, useState } from 'react'
import { Button } from '@/components/ui/button'
import { useUserStore } from '@/lib/user-store'
import { useRouter } from 'next/navigation'

declare global {
  interface Window {
    google: any
  }
}

interface GoogleSignInButtonProps {
  onSuccess?: () => void
  onError?: (error: string) => void
  disabled?: boolean
}

export default function GoogleSignInButton({ 
  onSuccess, 
  onError, 
  disabled = false 
}: GoogleSignInButtonProps) {
  const { loginWithGoogle, isLoading } = useUserStore()
  const router = useRouter()
  const [isGoogleLoaded, setIsGoogleLoaded] = useState(false)
  const [showDevWarning, setShowDevWarning] = useState(false)

  // Check if we're in development and Google OAuth is not properly configured
  const isDevelopment = process.env.NODE_ENV === 'development'
  const isLocalhost = typeof window !== 'undefined' && window.location.hostname === 'localhost'

  useEffect(() => {
    // Check if Google Client ID is available
    if (!process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID) {
      console.error('NEXT_PUBLIC_GOOGLE_CLIENT_ID is not set')
      onError?.('Google authentication is not configured')
      return
    }

    // Load Google Sign-In script
    const script = document.createElement('script')
    script.src = 'https://accounts.google.com/gsi/client'
    script.async = true
    script.defer = true
    document.body.appendChild(script)

    script.onload = () => {
      if (window.google) {
        try {
          window.google.accounts.id.initialize({
            client_id: process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID,
            callback: handleGoogleResponse,
            auto_select: false,
            cancel_on_tap_outside: true,
            // Add localhost as allowed origin for development
            allowed_parent_origin: ['http://localhost:3000', 'https://localhost:3000'],
          })
          setIsGoogleLoaded(true)
          console.log('Google Sign-In initialized successfully')
        } catch (error) {
          console.error('Failed to initialize Google Sign-In:', error)
          onError?.('Failed to initialize Google Sign-In')
        }
      }
    }

    script.onerror = () => {
      console.error('Failed to load Google Sign-In script')
      onError?.('Failed to load Google Sign-In')
    }

    return () => {
      if (document.body.contains(script)) {
        document.body.removeChild(script)
      }
    }
  }, [])

  const handleGoogleResponse = async (response: any) => {
    try {
      console.log('Google response received:', response)
      const result = await loginWithGoogle(response.credential)
      
      if (result.success) {
        onSuccess?.()
        router.push('/account')
      } else {
        onError?.(result.error || 'Google sign-in failed')
      }
    } catch (error) {
      console.error('Google sign-in error:', error)
      onError?.('Google sign-in failed')
    }
  }

  const handleGoogleSignIn = () => {
    if (!isGoogleLoaded) {
      onError?.('Google Sign-In is not ready yet')
      return
    }

    if (window.google) {
      try {
        // Check if we're in development mode and show helpful message
        if (process.env.NODE_ENV === 'development') {
          console.log('Development mode: If you see "unregistered_origin" error, you need to add http://localhost:3000 to your Google OAuth client authorized origins')
        }

        window.google.accounts.id.prompt((notification: any) => {
          console.log('Google prompt notification:', notification)
          if (notification.isNotDisplayed()) {
            const reason = notification.getNotDisplayedReason()
            console.error('Google prompt not displayed:', reason)
            
            if (reason === 'unregistered_origin') {
              onError?.('Google OAuth not configured for localhost. Please add http://localhost:3000 to authorized origins in Google Cloud Console.')
            } else {
              onError?.('Google sign-in popup was blocked or not displayed')
            }
          } else if (notification.isSkippedMoment()) {
            console.log('Google prompt skipped:', notification.getSkippedReason())
          }
        })
      } catch (error) {
        console.error('Error showing Google prompt:', error)
        onError?.('Failed to show Google sign-in')
      }
    } else {
      onError?.('Google Sign-In is not available')
    }
  }

  return (
    <>
      <Button
        type="button"
        variant="outline"
        className="w-full"
        onClick={handleGoogleSignIn}
        disabled={disabled || isLoading || !isGoogleLoaded}
      >
        <svg className="w-4 h-4 mr-2" viewBox="0 0 24 24">
          <path
            fill="currentColor"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
          />
          <path
            fill="currentColor"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          />
          <path
            fill="currentColor"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
          />
          <path
            fill="currentColor"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
          />
        </svg>
        {isGoogleLoaded ? 'Continue with Google' : 'Loading Google...'}
      </Button>
      
     
    </>
  )
}